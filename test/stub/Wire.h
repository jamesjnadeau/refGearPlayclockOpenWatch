...
// Host stub for the Arduino Wire library.
//
// PORTED FROM THE PARENT (watchy-ref-counter/tests/stub/Wire.h) with its
// header updated: this board fits one RTC, not two, so the PCF8563 half of
// the reason it exists is gone. What survives is the important half -- the
// RV-3028 holds the weekday at 0x03 and the date at 0x04, and a transposition
// there reads a PLAUSIBLE WRONG DATE rather than failing loudly.
//
...
// firmware most likely to be wrong and least likely to be caught by eye.
//
