...
//
// A fixed table in flash, one selected index held in RefStore, and a
// settings.h default used until the menu has been touched. The last entry,
// "Custom", is the only editable one; its numbers are stored too, so a reflash
// does not wipe them.
//
// PORTED FROM THE PARENT with NVS swapped for RefStore -- see that header for
// why this part cannot have Preferences and what emulated flash costs. The
// table, the clamps and the semantics are unchanged: the values follow the
// ReadyRef model line-up, and Football reproduces what the parent shipped
// with, so a watch that has never opened the menu behaves as it always did.
//
// The parent's header said "the shape here follows RefZone". RefZone is not
// ported (Task 24 Step 7), so that sentence is gone rather than left pointing
// at nothing.
//
