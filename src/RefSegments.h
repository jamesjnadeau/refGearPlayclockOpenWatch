...
//
// PORTED UNCHANGED from the parent (watchy-ref-counter). It is the one file
// that needed no work at all: it has no Arduino, GxEPD2 or panel-size
// dependency of any kind, and the panel it lays out for is a parameter. A
// 200 x 200 e-paper and a 128 x 128 memory LCD ask it the same question. That
// is what "pure arithmetic so it can be checked on a host" buys, and it is
// worth noticing which files paid off across a hardware change and which did
// not.
